...
def ranM(scdata, p, seedn):
    ...
    n = scdata.shape[1]  # number of samples/cells
...
def train_and_predict(tpm_test):
    subtype = pd.read_csv('/kaggle/input/subtype/subtype.csv',encoding='gbk')
    dind = subtype[subtype.iloc[:,4] == '0'].index
    subtype = subtype.drop(index= dind)
    tpm_train = pd.read_csv('/kaggle/input/ball-tpm/filter_HTSeq_TPM.csv')
